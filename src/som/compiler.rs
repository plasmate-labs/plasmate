...
fn resolve_label(
    ...
    if text.as_ref().is_none_or(|value| value.is_empty()) && is_icon_label_host(tag, attrs) {
        ...
        if let Some(title) = first_descendant_svg_title(node, css_rules) {
...
fn first_descendant_svg_title(node: &Handle, css_rules: &VisibilityRules) -> Option<String> {
    for child in node.children.borrow().iter() {
        if is_css_hidden_element(child, css_rules) {
            continue;
        }
        let NodeData::Element { name, .. } = &child.data else {
            continue;
        ...
        if child_attrs
            .iter()
            .any(|(n, v)| n == "aria-hidden" && v.eq_ignore_ascii_case("true"))
        {
            continue;
        }
        if child_attrs
        ...
        if tag == "svg" {
            ...
            for grandchild in child.children.borrow().iter() {
                if let NodeData::Element { name, .. } = &grandchild.data {
                    if name.local.as_ref() == "title" {
                        let title = heuristics::normalize_text(&get_visible_text_content(
                            grandchild, css_rules,
                        ));
                        if !title.is_empty() {
                            return Some(title);
                        }
        ...
        if let Some(found) = first_descendant_svg_title(child, css_rules) {
            return Some(found);
        }
    }
    None
...
<main>
  ...
  <input id="image-submit" type="image" src="/go.png" alt="Go">
    ...
    #[test]
    fn icon_button_and_link_use_descendant_svg_title() {
        let html = r#"<!DOCTYPE html>
<html><head><title>Toolbar</title></head>
<body>
<nav>
  <a id="home" href="/"><svg viewBox="0 0 8 8"><title>  Home  </title><circle cx="4" cy="4" r="3"></circle></svg></a>
  <a id="titled" href="/help" title="Tooltip"><svg viewBox="0 0 8 8"><title>Help</title><path d="M0 0"></path></svg></a>
</nav>
<main>
  <button id="close"><svg viewBox="0 0 8 8"><title>Close</title><path d="M0 0"></path></svg></button>
  <button id="save">Save <svg viewBox="0 0 8 8"><title>disk</title><path d="M0 0"></path></svg></button>
  <button id="aria" aria-label="Dismiss"><svg viewBox="0 0 8 8"><title>Close</title><path d="M0 0"></path></svg></button>
  <button id="empty"><svg viewBox="0 0 8 8"><title>   </title><path d="M0 0"></path></svg></button>
  <button id="hidden"><svg aria-hidden="true" viewBox="0 0 8 8"><title>Hidden</title><path d="M0 0"></path></svg></button>
  <button id="img-alt"><img src="/x.png" alt="From image"><svg viewBox="0 0 8 8"><title>From svg</title><path d="M0 0"></path></svg></button>
  <p id="caption">See <svg role="img"><title>Q3 revenue</title><circle cx="1" cy="1" r="1"></circle></svg> in EMEA.</p>
  <input id="image-submit" type="image" src="/go.png" alt="Go">
</main>
</body>
</html>"#;

        let som = compile(html, "https://example.test/svg-toolbar").unwrap();
        let mut elements = Vec::new();
        fn collect<'a>(nodes: &'a [Element], out: &mut Vec<&'a Element>) {
            for element in nodes {
                out.push(element);
        ...
        assert!(
            close.text.as_ref().is_none_or(|text| text.is_empty()),
            "icon button must not invent visible text from svg title: {close:?}"
        );
        assert_eq!(
            close.label.as_deref(),
            Some("Close"),
            "icon button must use descendant svg title as accessible name: {close:?}"
        );
        assert!(
            close
                .attrs
                .as_ref()
                .is_none_or(|attrs| attrs.get("title").is_none()),
            "button must not copy svg title onto attrs.title: {close:?}"
        );
        ...
        assert_eq!(
            home.label.as_deref(),
            Some("Home"),
            "icon link must use trimmed descendant svg title: {home:?}"
        );
        ...
        assert_eq!(
            titled.label.as_deref(),
            Some("Help"),
            "descendant svg title must win over title tooltip: {titled:?}"
        );
        ...
        assert_ne!(
            save.label.as_deref(),
            Some("disk"),
            "visible button text must not be replaced by nested svg title: {save:?}"
        );
        ...
        assert_eq!(
            aria.label.as_deref(),
            Some("Dismiss"),
            "aria-label must remain preferred over descendant svg title: {aria:?}"
        );
        ...
        assert!(
            empty.label.is_none(),
            "whitespace svg title must not become a name: {empty:?}"
        );
        ...
        assert!(
            ...
            "aria-hidden svg title must not become a name: {hidden:?}"
        );
        ...
        assert_eq!(
            ...
            Some("From image"),
            "descendant img alt must remain preferred over svg title: {img_alt:?}"
        );
...
<main>
  ...
  <input id="image-submit" type="image" src="/go.png" alt="Go">
    ...
    #[test]
    fn icon_button_and_link_use_descendant_img_aria_label() {
        let html = r#"<!DOCTYPE html>
<html><head><title>Toolbar</title></head>
<body>
<nav>
  <a id="home" href="/"><img src="/home.png" aria-label="  Home  "></a>
  <a id="titled" href="/help" title="Tooltip"><img src="/help.png" aria-label="Help"></a>
</nav>
<main>
  <button id="close"><img src="/x.png" aria-label="Close"></button>
  <button id="save">Save <img src="/disk.png" aria-label="disk"></button>
  <button id="aria" aria-label="Dismiss"><img src="/x.png" aria-label="Close"></button>
  <button id="empty"><img src="/spacer.png" aria-label="   "></button>
  <button id="hidden"><img src="/x.png" aria-hidden="true" aria-label="Hidden"></button>
  <button id="aria-wins"><img src="/x.png" alt="From alt" aria-label="From aria"></button>
  <button id="img-alt"><img src="/x.png" alt="From image"><svg aria-label="From svg" viewBox="0 0 8 8"><path d="M0 0"></path></svg></button>
  <p id="caption">See <img src="/q3.png" aria-label="Q3 revenue"> in EMEA.</p>
  <input id="image-submit" type="image" src="/go.png" alt="Go">
</main>
</body>
</html>"#;

        let som = compile(html, "https://example.test/img-aria-toolbar").unwrap();
        let mut elements = Vec::new();
        fn collect<'a>(nodes: &'a [Element], out: &mut Vec<&'a Element>) {
            for element in nodes {
                out.push(element);
        ...
        assert!(
            close.text.as_ref().is_none_or(|text| text.is_empty()),
            "icon button must not invent visible text from img aria-label: {close:?}"
        );
        assert_eq!(
            close.label.as_deref(),
            Some("Close"),
            "icon button must use descendant img aria-label as accessible name: {close:?}"
        );
        assert!(
            close.attrs.as_ref().is_none_or(|attrs| attrs
                .get("aria")
                .and_then(|aria| aria.get("label"))
                .is_none()),
            "button must not copy img aria-label onto attrs.aria.label: {close:?}"
        );
        ...
        assert_eq!(
            home.label.as_deref(),
            Some("Home"),
            "icon link must use trimmed descendant img aria-label: {home:?}"
        );
        ...
        assert_eq!(
            titled.label.as_deref(),
            Some("Help"),
            "descendant img aria-label must win over title tooltip: {titled:?}"
        );
        ...
        assert_ne!(
            save.label.as_deref(),
            Some("disk"),
            "visible button text must not be replaced by nested img aria-label: {save:?}"
        );
        ...
        assert_eq!(
            aria.label.as_deref(),
            Some("Dismiss"),
            "aria-label must remain preferred over descendant img aria-label: {aria:?}"
        );
        ...
        assert!(
            empty.label.is_none(),
            "whitespace img aria-label must not become a name: {empty:?}"
        );
        ...
        assert!(
            hidden.label.is_none(),
            "aria-hidden img aria-label must not become a name: {hidden:?}"
        );

        let aria_wins = elements
            .iter()
            .find(|element| element.html_id.as_deref() == Some("aria-wins"))
            .expect("img with alt and aria-label should compile");
        assert_eq!(
            aria_wins.label.as_deref(),
            Some("From aria"),
            "descendant img aria-label must win over alt: {aria_wins:?}"
        );

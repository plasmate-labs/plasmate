...
async fn send(state: &mut ConnectionState, method: &str, params: serde_json::Value) -> serde_json::Value {
...
async fn test_hello_handshake() {
    ...
    let resp = send(&mut state, "awp.hello", json!({
        "awp_version": "0.1",
        "client_name": "test"
    })).await;
...
async fn test_session_create_and_close() {
    ...
    let resp = send(&mut state, "session.create", json!({})).await;
    assert!(is_success(&resp));
    let session_id = get_result(&resp)["session_id"].as_str().unwrap().to_string();
    assert!(session_id.starts_with("s_"));

    let resp = send(&mut state, "session.close", json!({"session_id": session_id})).await;
    assert!(is_success(&resp));
...
async fn test_navigate_real_page() {
    ...
    let resp = send(&mut state, "page.navigate", json!({
...
async fn test_observe_after_navigate() {
    ...
    send(&mut state, "page.navigate", json!({
        "session_id": session_id,
        "url": "https://example.com"
    })).await;

    let resp = send(&mut state, "page.observe", json!({
        "session_id": session_id
    })).await;
...
async fn test_extract_structured_data() {
    ...
    let resp = send(&mut state, "page.extract", json!({
        "session_id": session_id,
        "structured_data": true
    })).await;

    assert!(is_success(&resp), "Structured data extraction should work: {:?}", resp);
    let result = get_result(&resp);
...
async fn test_extract_interactive_elements() {
    ...
    assert!(is_success(&resp));
    let result = get_result(&resp);
    assert!(result["count"].as_u64().unwrap() > 0, "example.com should have at least one link");
...
async fn test_act_click_link() {
    ...
    let resp = send(&mut state, "page.act", json!({
        "session_id": session_id,
        "intent": {
            "action": "click",
            "target": {
...
async fn test_act_type_into_field() {
    ...
    let extract_resp = send(&mut state, "page.extract", json!({
        "session_id": session_id,
        "interactive_elements": true
    })).await;

    let elements = get_result(&extract_resp)["interactive_elements"].as_array().unwrap();
...
async fn test_full_agent_workflow() {
    ...
    // 8. Close session
    let resp = send(&mut state, "session.close", json!({
        "session_id": &session_id
    })).await;
    assert!(is_success(&resp));
}

...
| `get_action_plan(som) -> list[dict]` | Return compact `{id, cache_key, role, actions, enabled, label}` action targets with availability, form/list and popover/command relation cues, input-affordance cues, validation constraints, and ARIA live-region cues |

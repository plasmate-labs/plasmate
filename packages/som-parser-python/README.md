...
| `get_action_plan(som) -> list[dict]` | Return compact `{id, cache_key, role, actions, enabled, label}` action targets with availability, form/list relation cues, input-affordance cues, validation constraints, and live state cues |

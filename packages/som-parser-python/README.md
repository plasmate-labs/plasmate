...
| `get_action_plan(som) -> list[dict]` | Return compact `{id, cache_key, role, actions, enabled, label}` action targets with availability, link target/rel/download cues, form/list and popover/command relation cues, input-affordance cues, validation constraints, ARIA live-region cues, and ARIA owns/flowto/details relationships |

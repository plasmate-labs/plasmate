...
// Compact action targets with id, cache_key, role, actions, enabled, labels, input hints, and validation cues.
...
| `getActionPlan(som): ActionPlanItem[]` | Return compact action targets with cache keys, availability, input-affordance cues, validation constraints, and live state cues for agent planning. |

...
// Compact action targets with id, cache_key, role, actions, enabled, labels, link target/rel/download cues, upload accept/capture/multiple cues, locale context, form/list and popover/command relation cues, submitter override cues, text-entry/input hints, validation/range cues, and ARIA owns/flowto/details plus orientation/sort/value state.

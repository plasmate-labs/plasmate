...
// Compact action targets with id, cache_key, role, actions, enabled, labels, and validation cues.
...
| `getActionPlan(som): ActionPlanItem[]` | Return compact action targets with cache keys, availability, validation constraints, and live state cues for agent planning. |

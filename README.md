...
- `DOM.getDocument`, `DOM.getAttributes`, `DOM.querySelector`, `DOM.querySelectorAll`
...
`#som_id`, `tag#id`, comma-separated selector lists, `[data-testid="..."]`,
`[data-test-id="..."]`, common attributes such as `[name="..."]`,
`[href="..."]`, `[type="..."]`, `[aria-label="..."]`,
`[aria-labelledby="..."]`, `[role="..."]`, SOM role names, text, and labels.
Matching for text and labels is case-insensitive. CDP DOM nodes expose replay
attributes such as `data-plasmate-id`, `data-som-role`, HTML id, test id, ARIA
label, href/name/type, and disabled/readonly/required flags when available
from `DOM.getDocument`, `DOM.describeNode`, and `DOM.getAttributes`.

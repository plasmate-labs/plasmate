...
compact click-target menu before the LLM sees the content. Use
`selector="text:checkout"` or `selector="label:email"` for human-facing
matches, and `selector="[data-testid=save]"`, `selector="input[name=q]"`, or
`selector="[required]"` when you already have developer-authored locator
anchors.

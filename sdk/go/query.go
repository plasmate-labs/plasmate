...
// FindUniqueActionTargetByLabel returns an exact label match only when the label maps to one action target.
func FindUniqueActionTargetByLabel(som *Som, label string, enabledOnly ...bool) *ActionPlanItem {
	index := GetActionPlanIndex(som, enabledOnly...)
	matches := index.ByLabelAll[label]
	if len(matches) == 1 {
		return &matches[0]
	}
	return nil
